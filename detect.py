...
import platform
import sys
from pathlib import Path  # Correctly import Path from pathlib
import telepot
import torch

# Create the bot instance
bot = telepot.Bot('7058162745:AAGH8Z293zXor3p3aMFvwtG4SLPxmBCiwKA')

# Set the path correctly based on the OS
if platform.system() == "Darwin":  # macOS
    temp = Path  # Use the default Path for macOS
else:
    temp = Path  # Use Path for all other systems as well
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_csv:  # Save to *.csv
                        prediction = names[int(cls)]
                        write_to_csv(p.name, prediction, conf.item())
            ...
            LOGGER.info(f"{s}Done. ({dt[0].dt:.3f}s)")
            ...
            if save_img:
                if dataset.mode == "images":
                    ...
                    vid_writer[i].write(im0)  # write video frame

    # Print results
    print(f"{colorstr('Results saved to ')}{save_dir}\n")  # print results
    if update:
        strip_optimizer(weights)  # update model (to fix Source in every epoch)
